...
Public Class frmEvoucherRep
    ...
    Private Sub GetEV_TreeView()
        Dim cmd_EV_CAT As String = "SELECT EVOUCHER_MASTER, LDESC FROM Centegy_SnDPro_UID.dbo.EV_EVOUCHER_MASTER"
        cmd_EV_CAT = "DECLARE @DATE DATE " _
            & "SET @DATE = '" & DTPEV.Value.ToString("yyyy-MM-dd") & "' " _
            & "SELECT B.EVOUCHER_MASTER,A.LDESC FROM Centegy_SnDPro_UID.dbo.EV_EVOUCHER_MASTER A LEFT JOIN Centegy_SnDPro_UID.dbo.EV_EVOUCHER_CATEGORY B " _
            & "ON A.EVOUCHER_MASTER=B.EVOUCHER_MASTER " _
            & "WHERE @DATE BETWEEN (B.FROM_DATE) AND (B.END_DATE)"

        Dim cmd_EV_MAS As String
        cmd_EV_MAS = "DECLARE @DATE DATE " _
            & "SET @DATE='" & DTPEV.Value.ToString("yyyy-MM-dd") & "' " _
            & "SELECT EVOUCHER_MASTER,EVOUCHER_CATEGORY,LDESC FROM Centegy_SnDPro_UID.dbo.EV_EVOUCHER_CATEGORY " _
    ...
    Public Sub Get_EV_HEAD()
        ...
        str_EV_HEAD = "select DISTRIBUTOR as DTCODE, NAME as DTNAME from Centegy_SnDPro_UID.dbo.DISTRIBUTOR"
    ...
    Private Sub GetComp_Dist_EVCAT()
        ...
        strcomp = "SELECT COMPANY FROM Centegy_SnDPro_UID.dbo.DISTRIBUTOR"
        strdist = "SELECT DISTRIBUTOR FROM Centegy_SnDPro_UID.dbo.DISTRIBUTOR"
    ...
    Public Sub Get_EV_CLAIM()
        ...
        cmd_EV_CLAIM = "DECLARE @START_DATE13 DATE, @END_DATE13 DATE, @START_DATE DATE, @COMPANY NVARCHAR(2), " _
            ...
            & "FROM Centegy_SnDPro_UID.dbo.JC_WEEK WHERE COMPANY+YEAR+JCNO+WEEK_NO < (SELECT COMPANY+YEAR+JCNO+WEEK_NO FROM Centegy_SnDPro_UID.dbo.JC_WEEK  WHERE @START_DATE >= START_DATE AND @START_DATE <= END_DATE)) A WHERE RN <= 13 " _
            ...
            & "(SELECT MIN(CA.DOC_NO) FROM Centegy_SnDPro_UID.dbo.CASHMEMO CA WHERE EP.COMPANY = CA.COMPANY AND EP.DISTRIBUTOR = CA.DISTRIBUTOR " _
            ...
            & "FROM Centegy_SnDPro_UID.dbo.EV_EVOUCHER_POP EP " _
            & "LEFT JOIN Centegy_SnDPro_UID.dbo.EV_EVOUCHER_ALLOCATION EA ON EP.EVOUCHER_CATEGORY = EA.EVOUCHER_CATEGORY " _
            & "LEFT JOIN Centegy_SnDPro_UID.dbo.RECEIPT_ADJ R ON EP.COMPANY = R.COMPANY AND EP.DISTRIBUTOR = R.DISTRIBUTOR AND EP.REF_DOCUMENT = R.DOCUMENT AND EP.REF_SUB_DOCUMENT = R.SUB_DOCUMENT AND EP.REF_DOC_NO = R.DOC_NO " _
            & "LEFT JOIN Centegy_SnDPro_UID.dbo.CASHMEMO CM ON R.COMPANY = CM.COMPANY AND R.DISTRIBUTOR = CM.DISTRIBUTOR AND R.REF_DOCUMENT = CM.DOCUMENT AND R.REF_SUB_DOCUMENT = CM.SUB_DOCUMENT AND R.REF_DOC_NO = CM.DOC_NO " _
            & "INNER JOIN Centegy_SnDPro_UID.dbo.EV_EVOUCHER_MASTER M ON EP.COMPANY = M.COMPANY AND EP.EVOUCHER_MASTER = M.EVOUCHER_MASTER " _
            & "INNER JOIN Centegy_SnDPro_UID.dbo.EV_EVOUCHER_CATEGORY EC ON EP.COMPANY = EC.COMPANY AND EP.EVOUCHER_MASTER = EC.EVOUCHER_MASTER " _
            & "AND EA.VALUE_COMB LIKE ('%~'+@DISTRIBUTOR) " _
            & "AND EP.EVOUCHER_CATEGORY = EC.EVOUCHER_CATEGORY INNER JOIN Centegy_SnDPro_UID.dbo.POP P ON EP.COMPANY = P.COMPANY AND EP.DISTRIBUTOR = P.DISTRIBUTOR " _
            ...
            & "SUM(COALESCE(CM.NET_AMOUNT,0)) / 13 LAST_13_WEEK_SALE FROM Centegy_SnDPro_UID.dbo.CASHMEMO CM WHERE CM.COMPANY = @COMPANY AND CM.DOC_DATE >= @START_DATE13 AND CM.DOC_DATE <= @END_DATE13 " _
    ...
    Public Sub Get_EV_Participant()
        ...
        cmd_EV_Participant = "DECLARE @DIST NVARCHAR(10) " _
            & "SELECT @DIST = (SELECT DISTRIBUTOR FROM Centegy_SnDPro_UID.dbo.DISTRIBUTOR) " _
            ...
            & "FROM Centegy_SnDPro_UID.dbo.EV_EVOUCHER_POP A " _
            & "LEFT JOIN Centegy_SnDPro_UID.dbo.POP B ON A.POP = B.POP " _
            & "LEFT JOIN Centegy_SnDPro_UID.dbo.POP_TYPE C ON B.POPTYPE=C.POPTYPE " _
            & "LEFT JOIN Centegy_SnDPro_UID.dbo.EV_EVOUCHER_CATEGORY D ON A.EVOUCHER_CATEGORY=D.EVOUCHER_CATEGORY " _
            & "LEFT JOIN Centegy_SnDPro_UID.dbo.EV_EVOUCHER_MASTER E ON A.EVOUCHER_MASTER = E.EVOUCHER_MASTER " _
            & "LEFT JOIN Centegy_SnDPro_UID.dbo.EV_EVOUCHER_ALLOCATION F ON A.EVOUCHER_CATEGORY = F.EVOUCHER_CATEGORY " _
